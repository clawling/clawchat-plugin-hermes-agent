...
| `ctx.register_tool(name, "clawchat", schema, handler, ...)`  | `clawchat_gateway.plugin_tools.register_tools` | Registers all thirty-two `clawchat_*` tools. List is also in `plugin.yaml`. |
