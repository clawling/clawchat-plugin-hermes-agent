...
| `ctx.register_tool(name, "clawchat", schema, handler, ...)`  | `clawchat_gateway.plugin_tools.register_tools` | Registers all thirty-one `clawchat_*` tools. List is also in `plugin.yaml`. |
