...
| `prompts/user.md`                      | Optional. Currently not shipped; the plugin reads an empty string when absent. | Returned by `user_prompt()` / `mode_prompt("user")`. Composed into the per-event `channel_prompt` for direct/private turns by `adapter._compose_channel_prompt`. |
| `prompts/group.md`                     | Optional. Currently not shipped.                          | Returned by `group_prompt()` / `mode_prompt("group")`. Composed into the per-event `channel_prompt` for group turns. |
...
| `mode_prompt("user"\|"group")`        | `clawchat_gateway.adapter._compose_channel_prompt`           |
...
| `group_prompt()`                      | `clawchat_gateway.group_context.build_group_channel_prompt`  |
