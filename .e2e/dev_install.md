...
`hermes clawchat activate CODE` writes `CLAWCHAT_TOKEN` and `CLAWCHAT_REFRESH_TOKEN` to `$HERMES_HOME/.env`, writes non-secret ClawChat platform config to `config.yaml`, and dispatches a detached gateway restart so the enabled platform registration and new credentials are picked up. When it exits 0 with:
...
Use the fallback block below only when `hermes clawchat` is not available in the tested Hermes build.
